...
export async function getAllEmployees(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    res.setHeader("Cache-Control", "public, max-age=600"); // cache employee directories for 10 minutes
...
export async function createEmployee(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const targetEmployee = emailConflict || codeConflict;
    ...
    if (targetEmployee) {
      ...
      if (normalizedAadhar) {
        const aadharConflict = await prisma.employee.findFirst({
          where: {
            ...
            NOT: { employee_code: targetEmployee.employee_code }
          }
        });
        if (aadharConflict) {
      ...
      // If the code is changing, execute a raw update first to handle primary key change
      if (emp.employee_code && emp.employee_code !== targetEmployee.employee_code) {
        await prisma.$executeRaw`UPDATE "Employee" SET employee_code = ${emp.employee_code} WHERE employee_code = ${targetEmployee.employee_code}`;
        targetEmployee.employee_code = emp.employee_code;
      }

      // Update existing employee
      const updatedEmp = await prisma.employee.update({
        where: { employee_code: targetEmployee.employee_code },
        data: {
          ...
          passport_history: emp.passport_history ? (JSON.parse(JSON.stringify(emp.passport_history)) as any) : undefined
        }
      });
      return res.status(200).json({ success: true, employee: updatedEmp });
    }
